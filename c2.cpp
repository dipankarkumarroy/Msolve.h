#include <iostream>
#include <string.h>
using namespace std;
class _Dip_
{
public:
    char data;
    _Dip_ *next;
};

class _Dip__s
{
public:
    float data;
    _Dip__s *next;
};

_Dip_ *char_top = NULL;
_Dip__s *start = NULL;

_Dip_ *create__Dip_(_Dip_ *ptr, char data)
{
    _Dip_ *new__Dip_;
    new__Dip_ = (_Dip_ *)malloc(sizeof(_Dip_));
    new__Dip_->data = data;
    new__Dip_->next = ptr;
    return new__Dip_;
}

void _Dip_push_(char data)
{
    char_top = create__Dip_(char_top, data);
}

char _Dip_pop_()
{
    char val = char_top->data;
    _Dip_ *ptr = char_top;
    if (char_top->next)
    {
        char_top = char_top->next;
    }
    else
    {
        char_top = NULL;
    }
    free(ptr);
    return val;
}

_Dip__s *create__Dip_2(_Dip__s *ptr, float data)
{
    _Dip__s *new__Dip_;
    new__Dip_ = (_Dip__s *)malloc(sizeof(_Dip__s));
    new__Dip_->data = data;
    new__Dip_->next = ptr;
    return new__Dip_;
}
void _Dip_push_2(float data)
{
    start = create__Dip_2(start, data);
}
float _Dip_pop_2()
{
    float val = start->data;
    _Dip__s *ptr = start;
    if (start->next)
    {
        start = start->next;
    }
    else
    {
        start = NULL;
    }
    free(ptr);
    return val;
}

class _Dip_queue_
{
public:
    float data;
    _Dip_queue_ *next;

    _Dip_queue_(float val) : data(val), next(nullptr) {}
};

class _queue_
{
private:
    _Dip_queue_ *front;
    _Dip_queue_ *rear;

public:
    _queue_() : front(nullptr), rear(nullptr) {}

    void _Dip_push_(float value)
    {
        _Dip_queue_ *new_Dip_queue_ = new _Dip_queue_(value);
        if (isEmpty())
        {
            front = rear = new_Dip_queue_;
        }
        else
        {
            rear->next = new_Dip_queue_;
            rear = new_Dip_queue_;
        }
    }

    float _Dip_pop_()
    {
        if (isEmpty())
        {
            cerr << "Missing value." << endl;
            return -1;
        }
        else
        {
            float _Dip_pop_pedValue = front->data;
            _Dip_queue_ *temp = front;
            front = front->next;
            delete temp;
            return _Dip_pop_pedValue;
        }
    }

    bool isEmpty() const
    {
        return front == nullptr;
    }
};

int _Dip_pri_(char s)
{
    if (s == '+' || s == '-')
    {
        return 0;
    }
    if (s == '*' || s == '/' || s == '%')
    {
        return 1;
    }
    if (s == '^')
    {
        return 2;
    }
    return -1;
}

string _Dip_InfTPo_(string s)
{
    string stack, postfix;
    char c;
    s = '(' + s + ')';
    for (int i = 0; i < s.size(); i++)
    {
        if (s[i] != '(' && s[i] != ')' && s[i] != '+' && s[i] != '-' && s[i] != '*' && s[i] != '/' && s[i] != '%' && s[i] != '^')
        {
            postfix = postfix + s[i];
            if (s[i + 1] == '(' || s[i + 1] == ')' || s[i + 1] == '+' || s[i + 1] == '-' || s[i + 1] == '*' || s[i + 1] == '/' || s[i + 1] == '%' || s[i + 1] == '^')
            {
                postfix = postfix + " ";
            }
        }
        else
        {
            if (s[i] == '(')
            {
                if (s[i + 1] == '+' || s[i + 1] == '-')
                {
                    postfix = postfix + "0 ";
                }
                _Dip_push_(s[i]);
            }
            else if (s[i] == ')')
            {
                do
                {
                    c = _Dip_pop_();
                    if (c != '(')
                    {
                        postfix = postfix + c + " ";
                    }
                } while (c != '(');
            }
            else
            {
                if (_Dip_pri_(char_top->data) == _Dip_pri_(s[i]))
                {
                    if (s[i] == '^')
                    {
                        _Dip_push_(s[i]);
                    }
                    else
                    {
                        c = _Dip_pop_();
                        while (_Dip_pri_(c) == _Dip_pri_(s[i]))
                        {
                            if (c != '(')
                            {
                                postfix = postfix + c + " ";
                            }
                            c = _Dip_pop_();
                        }
                        _Dip_push_(c);
                        _Dip_push_(s[i]);
                    }
                }
                else if (_Dip_pri_(char_top->data) < _Dip_pri_(s[i]))
                {
                    _Dip_push_(s[i]);
                }
                else if (_Dip_pri_(char_top->data) >= _Dip_pri_(s[i]))
                {

                    do
                    {
                        c = _Dip_pop_();
                        if (c != '(')
                        {
                            postfix = postfix + c + " ";
                        }
                    } while (c != '(');
                    _Dip_push_('(');
                    _Dip_push_(s[i]);
                }
            }
        }
    }
    return (" " + postfix);
}

float _Dip_cal_(string s)
{
    int i = 0;
    float val;
    string temp = "";
    s = s + 'E';
    while (s[i] == ' ' && s[i] != 'E')
    {
        if (s[i + 1] == '+' || s[i + 1] == '-' || s[i + 1] == '*' || s[i + 1] == '/' || s[i + 1] == '%' || s[i + 1] == '^')
        {
            float x, y, z;
            y = _Dip_pop_2();
            x = _Dip_pop_2();
            switch (s[i + 1])
            {
            case '^':
                z = 1;
                if (y == 0)
                {
                    z = 1;
                }
                else
                {
                    for (int i = 0; i < y; i++)
                    {
                        z = z * x;
                    }
                }
                break;
            case '*':
                z = x * y;
                break;
            case '/':
                z = x / y;
                break;
            case '%':
                z = (int)x % (int)y;
                break;
            case '+':
                z = x + y;
                break;
            case '-':
                z = x - y;
                break;
            default:
                break;
            }
            _Dip_push_2(z);
            i++;
        }
        else if (s[i + 1] != 'E')
        {
            while (s[i + 1] != ' ')
            {
                temp = temp + s[i + 1];
                i++;
            }
            val = stof(temp);
            _Dip_push_2(val);
            temp = "";
        }
        i++;
    }
    return _Dip_pop_2();
}

template <typename... Args>
string _Dip_conv_(const string &s, Args... args)
{
    int k = 0;
    string ts;
    _queue_ my_queue_;
    (my_queue_._Dip_push_(args), ...);
    while (s[k] != '\0')
    {
        if (s[k] == ' ')
        {
            k++;
        }
        if (isalpha(s[k]) || s[k] == '_')
        {
            ts = ts + to_string(my_queue_._Dip_pop_());
            while (isalpha(s[k]) || s[k] == '_' || s[k] == ' ' || isdigit(s[k]))
            {
                k++;
            }
        }
        else if (!isalnum(s[k]) || isdigit(s[k]))
        {
            ts = ts + s[k];
            k++;
        }
        else
        {
            k++;
        }
    }

    return ts;
}

float Msolve(string s)
{
    return _Dip_cal_(_Dip_InfTPo_(s));
}

template <typename... Args>
float Msolve(string s, Args... args)
{
    s = _Dip_conv_(s, args...);
    return _Dip_cal_(_Dip_InfTPo_(s));
}
int main()
{
    int x0 = 78;
    string x1 = to_string(x0);
    cout << Msolve("-1-2-3+4+3+5-08+78+54") << "\n";
    cout << Msolve("-1-2-3+4+3+5-08+" + to_string(x0) + "+54") << "\n";
    cout << Msolve("-1-2-3+4+3+5-08+" + x1 + "+54") << "\n";
    cout << Msolve("(ab2+ d_y tc-r+2.0)", 1, 2.5, 3) << '\n';
}
